...
export class RuleResultStore extends ComponentStore<Store> {
    ...
    get errors$(): Observable<RuleSet[]> {
        return this.select(state => this.filterByStatus(state.ruleSets, RuleStatus.ERROR));
    }

    get sucess$(): Observable<RuleSet[]> {
        return this.select(state => this.filterByStatus(state.ruleSets, RuleStatus.SUCCESS));
    }
